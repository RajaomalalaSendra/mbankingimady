...
5.times do |i|
	User.create(email: user_mail[i], password: "mememe", password_confirmation: "mememe")
	Amount.create(amount: amounts[i],  fee: fees[i], reference: reference[i], operator: operator[i], adress: adress[i], drawer: drawer[i], number: number[i], user_id: i + 1)
end